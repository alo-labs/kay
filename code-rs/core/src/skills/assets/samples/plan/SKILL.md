---
name: plan
description: Generate a plan for how an agent should accomplish a complex coding task. Use when a user asks for a plan, and optionally when they want to save, find, read, update, or delete plan files in $KAY_HOME/plans (default ~/.kay/plans).
metadata:
...
- Resolve the plans directory as `$KAY_HOME/plans` when `KAY_HOME` is set, otherwise `~/.kay/plans`.
...
```bash
python ./scripts/read_plan_frontmatter.py ~/.kay/plans/codex-rate-limit-overview.md
```
