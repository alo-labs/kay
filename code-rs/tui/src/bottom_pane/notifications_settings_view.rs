    ...
    fn toggle(&mut self) {
        match &mut self.mode {
            ...
            NotificationsMode::Custom { entries } => {
                ...
                self.app_event_tx.send_background_event_with_ticket(
                    &self.ticket,
                    "Edit ~/.kay/config.toml [tui].notifications to change filters.".to_string(),
                );
            }
        }
    }
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        let footer = match &self.mode {
            ...
            NotificationsMode::Custom { .. } => Line::from(vec![
                ...
                Span::styled(" in ~/.kay/config.toml to adjust filters.", Style::default().fg(crate::colors::text_dim())),
            ]),
        };
        lines.push(footer);
