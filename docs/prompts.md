...
- Location: Put files in `$CODE_HOME/prompts/` (defaults to `~/.code/prompts/`; Kay also reads the host `~/.codex/prompts/` overlay).
...
- How to use:
  - Start a new session (Kay loads custom prompts on session start).
...
- Notes:
  ...
  - New or changed files are discovered on session start. If you add a new prompt while Kay is running, start a new session to pick it up.
