...
Kay can launch external CLI “agents” and orchestrate them in multi-agent “subagent” flows such as `/plan`, `/solve`, and `/kay`.
...
```toml
[[subagents.commands]]
name = "plan"                     # slash name (/plan, /solve, /kay, or custom)
read_only = true                  # default plan/solve=true, kay=false
agents = ["code-gpt-5.4", "claude-opus-4.6"]  # falls back to enabled agents or built-ins
orchestrator_instructions = "Guidance for Kay before spawning agents"
...
- `orchestrator_instructions`: appended to the Kay-side prompt before issuing `agent.create`.
...
- Subagent editor: configure per-command agent lists, read-only flag, and instructions. Built-in `/plan` `/solve` `/kay` can be overridden the same way.
...
- Kay loads AGENTS.md files along the path (global, repo root, cwd) up to 32 KiB total; deeper files override higher-level ones.
