...
mod tests {
    ...
    fn sh_lc_sed_is_read_command() {
        let inner = "sed -n '10,20p' kay-rs/tui/src/chatwidget.rs";
        assert_parsed(
            &vec_str(&["sh", "-lc", inner]),
    ...
    fn supports_rg_with_path_and_glob_filter() {
        let inner = "rg \"codex-run-as-apply-patch\" -n kay-rs -g\"*.rs\"";
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Search {
                cmd: "rg codex-run-as-apply-patch -n kay-rs '-g*.rs'".to_string(),
                query: Some("codex-run-as-apply-patch".to_string()),
                path: Some("kay-rs".to_string()),
            }],
        );
    }
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd kay-rs && rg --files"),
            vec![ParsedCommand::Search {
                cmd: "rg --files".to_string(),
                query: None,
                path: None,
