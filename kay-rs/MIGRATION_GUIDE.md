# Crate Migration Guide: codex-rs to kay-rs

This guide documents the process for moving shared crates from `codex-rs` into `kay-rs`, making them fully independent.

## Overview

The `kay-rs` repository currently contains thin wrapper crates that re-export functionality from their `codex-rs` counterparts. This creates a cross-repository dependency via relative paths (`../codex-rs/...`). The migration process moves the implementation into `kay-rs` so that the crate owns its code and no longer depends on `codex-rs`.
...
- **Destination crate location**: `/home/azureuser/code/kay-rs/<crate-name>/`
- **Current wrapper structure**: Usually just `pub use codex_<crate>::*;`
- **Dependencies**: What other crates does this depend on?
- **Consumers**: What crates in kay-rs depend on this wrapper?

### 2. File Structure Migration

Copy all implementation files from codex-rs to kay-rs:

```bash
# Source modules (excluding lib.rs which needs special handling)
cp codex-rs/<crate>/src/*.rs kay-rs/<crate>/src/
# Exception: main.rs will need modification

# Tests (if they exist)
cp -r codex-rs/<crate>/tests/ kay-rs/<crate>/tests/
```
...
find kay-rs/<crate>/src -name '*.rs' -exec sed -i 's/codex_core/code_core/g' {} +

# Update codex_<crate> to code_<crate> (with underscores)
find kay-rs/<crate>/src -name '*.rs' -exec sed -i 's/codex_linux_sandbox/code_linux_sandbox/g' {} +
```
...
**Location:** `/home/azureuser/code/kay-rs/Cargo.toml`
...
The script will:
1. Copy source files from codex-rs to kay-rs
2. Copy test files
...
**Solution:** Ensure ALL dependencies from the codex-rs Cargo.toml are copied to kay-rs Cargo.toml, including:
...
```bash
grep -r "codex_" kay-rs/<crate>/src/
grep -r "codex_" kay-rs/<crate>/tests/
```
...
**Note:** Run `grep -r "codex-" kay-rs/*/Cargo.toml` to see current wrapper dependencies.
...
1. **Copying** implementation files from codex-rs to kay-rs
...
The result is a crate that owns its implementation and no longer depends on `../codex-rs`, making kay-rs more independent and easier to maintain.
