...
impl WidgetRef for &TrustDirectoryWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                Span::styled("You are running Kay in ", success_style),
                Span::styled(
        ...
        if self.is_git_repo {
            lines.push(Line::from(
                "  Since this folder is version controlled, you may wish to allow Kay",
            ));
            lines.push(Line::from(
        ...
        if self.is_git_repo {
            lines.push(create_option(
                ...
                "Yes, allow Kay to work in this folder without asking for approval",
            ));
            lines.push(create_option(
                1,
            ...
            lines.push(create_option(
                0,
                TrustDirectorySelection::Trust,
                "Allow Kay to work in this folder without asking for approval",
            ));
            lines.push(create_option(
                1,
                TrustDirectorySelection::DontTrust,
