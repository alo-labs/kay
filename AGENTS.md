...
# Rust/kay-rs and codex-rs

In the `kay-rs` folder where the active Kay Rust code lives:

- Crate names remain prefixed with `code-` for this path-only phase. For example, the `core` folder's crate is named `code-core`.
- When using format! and you can inline variables into {}, always do that.
- Treat `codex-rs` as a read-only mirror of `openai/codex:main`; edit Rust sources under `kay-rs` instead.
...
- All Auto Drive escape routing lives in `kay-rs/tui/src/chatwidget.rs`. The
...
- The approval pane must *never* swallow Esc. `kay-rs/tui/src/bottom_pane/auto_coordinator_view.rs`
...
- The VT100 harness lives under `kay-rs/tui/tests/vt100_chatwidget_snapshot.rs`. It renders the live `ChatWidget` UI into a `Terminal<VT100Backend>` so snapshots capture the exact PTY output the user sees (including frame chrome, composer rows, and streaming inserts).
...
- When you intentionally change rendering, review the `.snap.new` files that appear in `kay-rs/tui/tests/snapshots/` and accept them with `cargo insta review` / `cargo insta accept` (limit to this test where possible).
...
- After a successful Release workflow on `main`, `scripts/wait-for-gh-run.sh` automatically runs `scripts/upgrade-local-kay-latest.sh` and verifies the visible `kay` command is the latest published version. Bypass only for diagnostics with `KAY_SKIP_LOCAL_UPGRADE=1`.
- The Google Chat release announcement is a required release step. Confirm the `Announce release in Google Chat` job completed successfully in the Release workflow; if it did not run or failed, use the `Announce Google Chat Release` workflow for the released tag before calling the release done.
- Include the Google Chat announcement text in the final response here so the release communication is visible in the session transcript.
