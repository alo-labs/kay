...
KAY_RS_DIR="$ROOT_DIR/kay-rs"
FORBIDDEN_DIR="$ROOT_DIR/codex-rs"

if [[ ! -d "$KAY_RS_DIR" || ! -d "$FORBIDDEN_DIR" ]]; then
  echo "ERROR: Expected both kay-rs/ and codex-rs/ to exist next to this script." >&2
  exit 1
fi

violations=0

echo "Scanning Cargo manifests under kay-rs/ for forbidden ../codex-rs references…"
...
done < <(find "$KAY_RS_DIR" -name Cargo.toml -print0)

if command -v jq >/dev/null 2>&1; then
  echo "Running cargo metadata guard…"
  metadata=$(cd "$KAY_RS_DIR" && cargo metadata --format-version 1 --all-features 2>/dev/null)
  if [[ -n "$metadata" ]]; then
    offenders=$(jq -r \
...
echo "✅ No forbidden ../codex-rs dependencies detected in kay-rs/."
