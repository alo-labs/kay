...
if (isWin) {
  tryExec('taskkill /IM kay-x86_64-pc-windows-msvc.exe /F');
...
//    @alo-labs scope, which npm will reuse (e.g., .kay-XXXXX). Remove only
...
try {
  ...
  try {
    ...
    scopeDir = path.join(root, '@alo-labs');
  } catch {
  ...
  if (existsSync(scopeDir)) {
    ...
    for (const name of readdirSync(scopeDir)) {
      if (!name.startsWith('.kay-') && !name.startsWith('.code-')) continue;
