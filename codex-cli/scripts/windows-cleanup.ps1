...
taskkill /IM kay-x86_64-pc-windows-msvc.exe /F 2>$null | Out-Null
taskkill /IM kay.exe /F 2>$null | Out-Null
taskkill /IM coder.exe /F 2>$null | Out-Null

Write-Host "Removing old global package (if present)..."
$npmRoot = (& npm root -g).Trim()
$pkgPath = Join-Path $npmRoot "@alo-labs\kay"
...
Get-ChildItem -LiteralPath (Join-Path $npmRoot "@just-every") -Force -ErrorAction SilentlyContinue |
  Where-Object { $_.Name -like '.kay-*' -or $_.Name -like '.code-*' } |
  ForEach-Object {
