#!/usr/bin/env node
// Unified entry point for the Kay CLI (fork of OpenAI Codex).
...
// When users run via `npx @alo-labs/kay`, we must always execute our
...
const tryBootstrapBinary = async () => {
  try {
    ...
    try {
      ...
      const name = (() => {
        if (platform === "win32") return "@alo-labs/kay-win32-x64"; // may be unpublished; falls through
        const plt = nodePlatform();
        const cpu = nodeArch();
...
if (existsSync(binaryPath)) {
  ...
  console.error(`  npm uninstall -g @alo-labs/kay`);
  console.error(`  npm install -g @alo-labs/kay`);
  if (isWSL()) {
    console.error("Detected WSL. Install inside WSL (Ubuntu) separately:");
    console.error("  npx -y @alo-labs/kay@latest  (run inside WSL)");
...
if (!validation.ok) {
  ...
  if (isWSL()) {
    ...
    console.error("  npx -y @alo-labs/kay@latest  (inside WSL)");
  }
  process.exit(1);
}
...
try {
  ...
  if (isNpx && process.stderr && process.stderr.isTTY) {
    ...
    try {
      ...
      if (line && !line.includes("@alo-labs/kay")) {
        otherCode = line;
      }
    } catch {}
    if (otherCode) {
      console.error(`@alo-labs/kay: running bundled binary -> ${binaryPath}`);
...
child.on("error", (err) => {
  ...
  if (code === 'EACCES') {
    ...
    console.error(`Or reinstall the package with: npm install -g @alo-labs/kay`);
    ...
    console.error("  npm uninstall -g @alo-labs/kay && npm install -g @alo-labs/kay");
    if (platform === 'win32') {
    ...
    if (isWSL()) {
      ...
      console.error("Install inside WSL and run there: npx -y @alo-labs/kay@latest");
    }
  } else {
    console.error(err);
  }
